...
object EjsNTransformSolver {
    ...
    private suspend fun getOrCreateSolver(): SolverWebView? {
        solverWebView?.let { cached ->
            if (cached.nFunctionAvailable) return cached
            solverWebView = null
            Timber.tag(TAG).d("Cleared stale EJS solver (n-function not available), will retry")
        }

        return withContext(NonCancellable) {
            solverWebView?.let { cached ->
                if (cached.nFunctionAvailable) return@withContext cached
                solverWebView = null
            }
        ...
        init {
            ...
            webView.webChromeClient = object : WebChromeClient() {
                override fun onConsoleMessage(m: ConsoleMessage): Boolean {
                    val msg = m.message()
                    if (msg.contains("Uncaught") || msg.contains("Failed to load") || msg.contains("ERR_") || msg.contains("error") || m.messageLevel() == ConsoleMessage.MessageLevel.ERROR) {
                        Timber.tag(TAG).e("EJS WebView: $msg at ${m.sourceId()}:${m.lineNumber()}")
                    } else if (m.messageLevel() == ConsoleMessage.MessageLevel.WARNING) {
                        Timber.tag(TAG).w("EJS WebView: $msg at ${m.sourceId()}:${m.lineNumber()}")
                    }
...
function transformN(nValue) {
    try {
        ...
        EjsBridge.onNError(e.toString() + '\n' + (e.stack || ''));
            ...
            val htmlFile = File(cacheDir, "solver.html")
