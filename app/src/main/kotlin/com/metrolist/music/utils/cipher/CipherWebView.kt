    ...
    if (!nFuncName) {
        try {
            ...
            for (var i = 0; i < keys.length; i++) {
                try {
                    var key = keys[i];
                    if (key.startsWith("webkit") || key === "CipherBridge" || key === "_cipherSigFunc" || key === "_nTransformFunc" || key === "window") continue;
                    var fn = window[key];
                    if (typeof fn !== 'function' || fn.length !== 1) continue;
                    
                    // Exclude native functions
                    if (fn.toString().indexOf('[native code]') >= 0) continue;

                    tested++;
                    var result = fn(testInput);
                    if (typeof result === 'string' && result !== testInput && result.length >= 10 && result.length <= 24) {
                        candidates.push(key + ":" + result.substring(0, 50));
                        // Modern n-parameter results are usually ~12-20 chars. 
                        // Old ones contained "_w8_". 
                        if (result.indexOf('_w8_') >= 0 || (result.length >= 12 && result.length <= 20)) {
            ...
            if (!nFuncName && candidates.length > 0) {
                ...
                // If we didn't find a "perfect" match but have candidates, pick the first one as a last resort
                var bestCandidate = candidates[0].split(':');
                if (bestCandidate[1].length >= 10 && bestCandidate[1].length <= 24) {
                   var pick = bestCandidate[0];
                   window._nTransformFunc = window[pick];
                   nFuncName = pick;
                   info += ",picked_first_candidate=" + pick;
                }
            }
        } catch(e) {
